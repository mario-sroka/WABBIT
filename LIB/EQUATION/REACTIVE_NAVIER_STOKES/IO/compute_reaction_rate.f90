...
! WABBIT
! ============================================================================================
!> \name compute_reaction_rate.f90
!> \version 0.5
!> \author msr
!
!> \brief compute reaction rate with cantera function 
!>
...
subroutine compute_reaction_rate( params_physics, phi, phi_work, gas )
     ...
     if (params_physics%d == 2) then
         ...
         do j = 1, Bs(1)+g+g
             do i = 1, Bs(2)+g+g
                 ...
                 ! reaction rate from cantera
                 call getNetProductionRates(gas, dummy2)
                 phi_work(i, j, 1) = sum(params_physics%dh(:) * dummy2(:) * params_physics%Wk(:))
         ...
         do j = 1, Bs(2)+g+g
             do i = 1, Bs(1)+g+g
                 do k = 1, Bs(3)+g+g
                     ...
                     ! reaction rate from cantera
                     call getNetProductionRates(gas, dummy2)
                     phi_work(i, j, k) = sum(params_physics%dh(:) * dummy2(:) * params_physics%Wk(:))
